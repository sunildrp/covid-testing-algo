                               ...
                               mainPanel(
                                   tabsetPanel(id = "panels",
                                               tabPanel(h4(strong("About")), value = "about", 
                                                        ...
                                                        br(),
                                                        p("Given the population being tested and the prevalence of the disease under consideration, this application predicts the sensitivities and specificities of a combination of tests, predictive values, number of second tests required, and average time to results in the following scenarios: "),
                                                        p(strong("1. Baseline scenario:"), "Either one of the two tests is applied alone as the only diagnostic tool."),
                                                        p(strong("2. Sequential testing:"), "Two tests are applied one after another to confirm the result of the first test."),
                                                        p(strong("2a. Confirmatory testing for positives:"), "The second test is applied only if the first test result is positive. This approach optimizes the specificity of combination testing, meaning you are aiming to minimise false-positive results."),
                                                        p(strong("2b. Confirmatory testing for negatives:"), "The second test is applied only if the first test is negative. This approach optimizes the sensitivity of combination testing, meaning you are aiming to minimise false-negative results."),
                                                        hr(),
                                                        h4(strong("Definitions:")),
                                                        br(),
                                                        p(strong("1.	Average time to results:"), "This is the average turnaround time for a testing strategy from the time of sample collection to the time of reporting of results to the subject from whom the samples were collected."),
                                                        ...
                                                        p(strong("3.	Negative predictive value (NPV):"), "NPV is the probability that following a negative test result, the individual will truly not have the disease."),
                                                        ...
                                                        p(strong("4.	Sensitivity:"), "Sensitivity is the proportion of the population who test positive among all those who actually have the disease. "),
                                                        p(strong("5.	Specificity:"), "Specificity is the proportion of the population who test negative among all those who do not actually have the disease. "),
                                                        p("   A highly sensitive test helps to rule out a disease when the test is negative, and a highly specific test helps to rule in a disease when the test is positive. "),
                                                        hr(),
                                                        h4(strong("Limitations:")),
                                                        br(),
                                                        p("Due to a lack of diagnostic accuracy data, the current version of the application does not account for any potential differences in test performances depending on the test population (asymptomatic versus symptomatic; acute versus screening). The input data for test accuracy estimates were obtained from independent clinical studies of symptomatic infections. "),
                                                        p("The average time to results is a function of inputs for the time to results for each test. It is suggested that the input parameter for this turnaround time reflects the total time taken for a subject to be informed of a test result, form the time of sample collection and incorporating delays due to shipping, batching of samples and reporting delays. The average output time to results should be interpreted based on the assumptions you have made while selecting the input parameters for the time to results. "),
                                                        p("The user should consider that the default parameters of performance characteristics (sensitivity and specificity) of each Ag-based test in the model are relative to those of RT-PCR and might not reflect thier true performance in the population. When combining Ag-tests and RT-PCR, we empirically selected the input parameters for the test performance of Ag-tests and RT-PCR taking into consideration the available knowledge to reflect the true performance of these tests in the population."),
                                                        hr(),
                                                        p(strong("Authors")),
    ...
    observe({
        showNotification(id = "disclaimer_message", 
                         p(h3(strong ("Disclaimer")),
                           br(),
                           "While every effort has been taken during the development of this tool/model for it to be as accurate and reliable as possible, it is important that the user understands that the outputs are a prediction based on the assumptions chosen through the input parameter values.", 
                           br(),
                           br(),
                           "The appropriate use of this tool/model and its output can contribute to effective policymaking, but misuse or misinterpretation of the output can mislead decision-making. Any decisions taken while using these tools are the responsibility of the user and no liability whatsoever will be taken by the developers/authors of the tool."
                           , br(),
                           br(),
                         ),
